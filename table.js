...
const menus = [
	{name: "짜장면", price: 3000, kcal: 300},
	{name: "짬뽕", price: 3500, kcal: 350},
	{name: "울면", price: 4000, kcal: 200},
	{name: "특수짜장", price: 3800, kcal: 340},
];
...
const OrderTable = (menus) => {
	...
	const trList = document.getElementById("menu-tbody").children; // static collection으로 루프를 돌리는 것이 원하는 방법으로 동작한다.
	menus.map((menu, menu_idx) => {
		const tr = trList[menu_idx].children;
...
const renderTable = (menus) => {
	...
	fields.forEach((field, idx) => {
		if (idx > 0) {
			footerEls[idx].innerText = total[field];
		}
	});
	// 총 가격
};
...
function init() {
	const colHeads = document.getElementById("menu-thead");
	renderTable(menus);
	colHeads.addEventListener("click", renderTableHandler);
}
